...
def optimalZeropad(x, fs, f):
    ...
    """
    M = x.size                         #Define size of DFT as size of input signal
    a = fs/f                           #Samples per period e.g. 10
    zpad = (math.ceil(M/a)-(M/a))*a         #Determine the amount to zero pad
    xpad = np.append(x,np.zeros(zpad)) #Zero pad the signal x
    X = fft(xpad)
