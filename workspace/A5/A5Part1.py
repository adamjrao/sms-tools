...
def minFreqEstErr(inputFile, f):
    ...
    #Import file and select area to analyze
    (fs, x) = UF.wavread(inputFile)
    xsize = x.size
    Mrange = 0
    
    def window_step (start, end,output):
        while output < end:
            output = start*100 + 1
            start = start + 1
            yield output
    ...
    for M in window_step(1,2000,1):
        Mnext = M
        Mrange = np.append(Mrange,Mnext)
    ...
    while error > 0.05:
        M = Mrange[i]
        N = next_greater_power_of_2(M)
        w = get_window(window, M)
        x1 = x[0.5*fs-(np.floor(M/2)):0.5*fs+(np.ceil(M/2)+1)]
