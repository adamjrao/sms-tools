...
def extractMainLobe(window, M):
    ...
    hM1 = int(math.floor((M+1)/2))    # find middle of window
    hM2 = int(math.floor(M/2))

    N = M*8                           # zero phase window + zero pad the window
    hN = int(math.floor((N+1)/2))     # find middle of window
    fftbuffer = np.zeros(N)
    fftbuffer[:hM1] = w[hM2:]
    fftbuffer[N-hM2:] = w[:hM2]

    X = fft(fftbuffer)                # obtain the magnitude of the spectrum and set minimum value to eps
    absX = abs(X)
    ...
    #test = argrelextrema(mX1, np.less, order=radius)
    #plt.scatter(np.arange(0,mX1.size),mX1)
    
    #locate = abs(np.diff(np.diff(mX1)))
    #locate2 = np.argwhere(locate<1)
    #start = locate2[0]
    #end = locate2[locate2.size-1]
    ...
    if window in ['hamming']:
        dictionary = loadTestCases.load(1,3)
        output = dictionary['output']
        test = mX1[1008:1041]
