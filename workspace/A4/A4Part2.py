...
def computeSNR(inputFile, window, M, N, H):
    ...
    #Calculate SNR1 (SNR over entire length of the signals)
    noise = np.subtract(x,y)
    energy_noise = np.sum(np.square(noise))
    energy_x = np.sum(np.square(x))
    SNR1 = 10*np.log10(energy_x/energy_noise)

    #Calculate SNR2 (discard M samples from start and end)
    noise2 = x[M:x.size-M]-y[M:x.size-M]
    energy_noise2 = np.sum(np.square(noise2))
    energy_x2 = np.sum(np.square(x[M:x.size-M]))
